...
export const Route = createFileRoute("/operacao")({
  head: () => ({
    meta: [
      { title: "Operação — Casa Almeria" },
      { name: "robots", content: "noindex, nofollow" },
    ],
  }),
  component: OperacaoPortalPage,
});

type PortalState = "idle" | "resolvendo" | "erro";

function OperacaoPortalPage() {
  ...
  const redirecionarParaCentral = useCallback(async () => {
    ...
    try {
      ...
      if (!token) {
        setPortalState("erro");
        setPortalErro(
          "Não foi possível abrir a central de pedidos. Peça a um administrador para configurar o módulo Operação.",
        );
        return;
      }
  ...
  useEffect(() => {
    ...
    if (isCozinha && !isAdmin && !isModoOperacaoRestrita) return;
    if (isModoOperacaoRestrita || isAdmin) {
      void redirecionarParaCentral();
    }
  }, [loading, user, isModoOperacaoRestrita, isCozinha, isAdmin, redirecionarParaCentral]);

  if (loading || (user && canAccessPedidos && portalState === "resolvendo")) {
    return (
  ...
  if (user && isCozinha && !isAdmin && !isModoOperacaoRestrita) {
    return (
      <>
        <AccessDenied
          title="Use o módulo Cozinha"
          description="Sua conta tem acesso à cozinha completa. Acesse /cozinha em vez deste portal."
          showSignOut
  ...
  if (user && !canAccessPedidos) {
    return (
      <>
        <AccessDenied
          title="Acesso restrito"
          description="Esta área é exclusiva para a equipe de operação. Use o painel administrativo se você for gestor."
          showSignOut
          onSignOut={() => supabase.auth.signOut()}
        />
        <Toaster position="bottom-right" />
  ...
  if (user && canAccessPedidos && portalState === "erro") {
    return (
      <>
