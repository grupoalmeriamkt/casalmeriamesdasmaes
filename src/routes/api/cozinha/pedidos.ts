...
export const Route = createFileRoute("/api/cozinha/pedidos")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        if (shareErr) {
          console.error("[cozinha/pedidos] share_tokens error", shareErr);
        ...
        if (error) {
          console.error("[cozinha/pedidos] pedidos error", error);
          return Response.json({ error: "db_error" }, { status: 500 });
        }
